...
test.describe("Critical User Journeys", () => {
  ...
  test.skip(
    !canRunBrowser,
    "Skipping E2E journey because browser dependencies are missing.",
  );
  ...
  test("Login -> Create Asset -> Assign -> Transfer", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Welcome to Veylix" }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("heading", { name: "Assets Directory" }),
    ).toBeVisible();
